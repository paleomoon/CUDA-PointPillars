...
/*
ScatterBEV插件将一维数据转换为二维图像
原始的pointpillar预处理之后的bev特征要经过一系列操作(PointNet -> MaxPooling)后送入Scatter层生成伪图像
这里将MaxPooling（ReduceMax）操作部分并入了ScatterBEV插件
featureX、featureY：2D backbone输出特征尺寸
*/
...
{
    int pillar_idx = blockIdx.x * PILLARS_PER_BLOCK + threadIdx.x; //pillar id
    int valid_pillars_inBlock = PILLARS_PER_BLOCK; //一个block中的pillar数
    const int num_pillars = params_data[0];
    int valid_blocks = (num_pillars+PILLARS_PER_BLOCK-1)/PILLARS_PER_BLOCK; //block数
    if(blockIdx.x >= valid_blocks) return;
    if(blockIdx.x == (valid_blocks-1)) {
        valid_pillars_inBlock = num_pillars % PILLARS_PER_BLOCK;
    }
    ...
    __syncthreads();
    if(pillar_idx >= num_pillars) return;
    uint4 coord = ((const uint4 *)coords_data)[pillar_idx]; //coords_data：0,0,y,x
    unsigned int x = coord.w;
    unsigned int y = coord.z;
...
int pillarScatterFloatKernelLaunch(int max_pillar_num,
                                   ...
                                   cudaStream_t stream)
{
    dim3 blocks((featureX*featureY+PILLARS_PER_BLOCK-1)/PILLARS_PER_BLOCK);//one thread one pillar
    dim3 threads(PILLARS_PER_BLOCK); //一个block中的thread数量
